...
def process_time_series(df, grouping=True):
    ...
    if grouping:
        ...
        df_resampled['value'] = df_resampled.interpolate(method='linear')
...
def period(time_diff):
    patterns = dict()
    if time_diff <= 300*3:
        patterns['first'] = int(3600 / time_diff)
    else:
        patterns['first'] = int(3*3600 / time_diff)
    patterns['second'] = int(24 * 3600 / time_diff)
    return patterns
